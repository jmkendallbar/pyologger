...
def generate_features_sequential(data, recording_start_datetime, data_type='EEG', sfreq=500, window_length_sec=3600, buffer_length_sec=300, epoch_size_sec=32,
    ...
    for i in range(0, len(data), window_size):
        ...
        features_subset['epoch_index'] = features_subset['epoch_index'] + indices_extra_window[0] // sfreq
        feature_dfs.append(features_subset[features_subset['epoch_index'].between(indices[0]//sfreq, indices[1]//sfreq, inclusive='both')])
...
def get_heart_rate(ecg_data, fs=500, search_radius=200, filter_threshold=200, algo='heartpy', correct_peaks=True):
    """
    ...
    def remove_spikes(signal, threshold=15):
        median = np.median(signal)
        mad = np.median(np.abs(signal - median))
        return np.where(np.abs(signal - median) > threshold * mad, median, signal)

    # sleepecg implementation
    if algo == 'sleepecg':
        cleaned_ecg = remove_spikes(bandpass_filter(ecg_data, lowcut=0.5, highcut=45, fs=fs), threshold=15)
        rpeaks = detect_heartbeats(cleaned_ecg, fs) # using sleepecg
        if correct_peaks == True:
            rpeaks_corrected = wfdb.processing.correct_peaks(
                cleaned_ecg, rpeaks, search_radius=search_radius, smooth_window_size=50, peak_dir="up"
            )
            rpeaks = rpeaks_corrected
        # MIGHT HAVE TO UPDATE search_radius
        heart_rates = [60 / ((rpeaks[i+1] - rpeaks[i]) / fs) for i in range(len(rpeaks) - 1)]
        # Create a heart rate array matching the frequency of the ECG trace
        hr_data = np.zeros_like(cleaned_ecg)
        ...
        filled = hr_data.isna().sum() // fs
        hr_data = hr_data.interpolate(method='linear').fillna('ffill').fillna('bfill')
        print('Filled', filled, 'bad heart rate values')
        return hr_data
    
    # heartpy implementation
    elif algo == 'heartpy':
        # Filter and remove spikes
        cleaned_ecg = remove_spikes(bandpass_filter(ecg_data, lowcut=0.5, highcut=45, fs=fs), threshold=15)
        working_data, measures = hp.process(cleaned_ecg, fs)
        rpeaks = np.array(working_data['peaklist'])
        if correct_peaks == True:
            rpeaks_corrected = wfdb.processing.correct_peaks(
                cleaned_ecg, rpeaks, search_radius=search_radius, smooth_window_size=50, peak_dir="up"
            )
        ...
        print('Unknown heart rate peak detection algorithm option')
