...
def create_state_event(state_df, key, value_column, start_time_column, duration_column=None, description=None, existing_events=None):
    """
    ...
    description : str, optional
        Short description for the state events (default is None).
    ...
    # Generate the new event DataFrame
    new_events = pd.DataFrame({
        ...
        'value': state_df[value_column],
        'type': 'state',
        'key': key,
        'duration': state_df[duration_column] if duration_column else np.nan,
        'short_description': description,
        'long_description': np.nan,  # Optional field, left blank
        'datetime': state_df[start_time_column]
    })

    # Concatenate new events with the existing events
